...
function makePlotWidget() {
    return {
        ...
        draw(ctx, node, widgetWidth, y, widgetHeight) {
            ...
            // With N steps there are N sample positions spread across the plot
            // (t = k/(N-1) for k=0..N-1) -- interior tick marks (excluding the
            // two already-marked box edges) sit at that same N-1 divisor, e.g.
            // the original hardcoded 8-step case used divisor 7.
            ...
            for (let i = 0; i <= 4; i++) {
                const yy = toY(-1 + (i / 4) * 2);
                ctx.beginPath();
                ctx.moveTo(x0 - 3, yy);
                ctx.lineTo(x0, yy);
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(x0 + plotW, yy);
                ctx.lineTo(x0 + plotW + 3, yy);
                ctx.stroke();
            }

            for (let i = 1; i < stepDivisor; i++) {
                const xx = toX(i / stepDivisor);
                ctx.beginPath();
                ctx.moveTo(xx, plotY);
                ctx.lineTo(xx, plotY - 4);
                ctx.stroke();
            }
            ...
            if (p.exponent != 0) {
                ...
                ctx.beginPath();
                ctx.moveTo(toX(mid), plotY);
                ctx.lineTo(toX(mid), plotY + plotH);
                ctx.stroke();
            }
            ctx.setLineDash([]);
